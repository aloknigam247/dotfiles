...
- **Utility extraction opportunities**: Scan the session for repeated multi-step operations that *should* become reusable scripts or skill helpers but weren't extracted. Look for: (a) the same 3+ tool calls (git commands, file reads, API calls) executed multiple times in the same pattern — especially across subagents; (b) manual data-gathering steps where several commands collected context that one script could produce; (c) boilerplate in agent prompts where each subagent independently runs the same setup commands. For each opportunity, describe what the utility would do, where it should live, and offer to create it.
